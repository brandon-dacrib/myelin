...
Prior: C, because it is the only one whose cost model matches the operations the protocol actually performs (compare two states, apply a small change, share across forks) and because it composes with room-owner memory and with object storage tiering. But the corpus decides.
