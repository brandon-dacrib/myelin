...
Last updated: 2026-09-18 (day one, session 2). This session picked up from session 1's draft RFC and OpenAPI skeleton and carried the track through the design freeze plus every Phase 0 deliverable except the WebAssembly host (deferred by design; see section "Decisions made").
...
- **`hs-admin-mock` does not enforce per-operation scopes**, only "is there a recognized bearer token at all". Real scope enforcement lives in `hs_admin::router` (deliverable 5) and is unit-tested there; re-implementing the same scope matrix in the mock would be a second, divergent copy of that logic for a server whose only job is to hand back realistic fixtures. Documented in `hs-admin-mock/main.rs`'s module doc comment.
...
cargo test -p hs-http -p hs-admin -p hs-modules                                 # 64 tests, all passing
