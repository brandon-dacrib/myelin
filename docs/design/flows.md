...
2. **Kind.** The catalogue (`GET /api/v1/bridge-types`, so 11 and 15 extend it without a UI release), grouped by what a person is connecting: Messaging (WhatsApp, Telegram, Signal, Google Messages, Google Voice, Messenger and Instagram), Communities and work (Discord, Slack, X, LinkedIn, Bluesky), IRC (heisenbridge, matrix-appservice-irc), Integrations (Hookshot). Each card: a glyph, the name, a one-line description, and what it needs (a phone, an API ID, an account). A search box narrows it; the chosen one's documentation link sits under the grid (a link inside a radio card is not valid HTML). Choosing a kind fills sensible defaults for every later step, including the port the bridge listens on.
...
4. **Deployment.** Two cards: **Kubernetes** (the operator will create a `Bridge` resource; asks for namespace, image tag with the latest known tag prefilled, database mode: own CloudNativePG database or shared cluster schema) and **Self-managed** (the operator runs the bridge themselves; produces the bridge's config, `registration.yaml` and a Docker Compose service). In single-node mode Kubernetes is hidden. Then the two addresses that make the bridge and the server find each other: **this server, as the bridge reaches it** (`homeserver.address` in the bridge's config: the Compose service name, the Kubernetes service, or `http://host.docker.internal:8008` from Docker to a server on the same machine) and **the bridge, as this server reaches it** (the registration's `url`: the bridge's service name on its port, or a published port). Both default from the deployment, the id and the namespace and follow them until the operator types into them.
5. **Options.** All on by default, each saying what it does to the registration and to the bridge's config rather than what one might hope: double puppeting (a non-exclusive claim on every local user in the registration, and the bridge's own token in its `double_puppet.secrets`), encryption (MSC3202 device lists and OTK counts, MSC4190 device management declared in the registration; `encryption.allow/default/appservice/msc4190` in the config), rate-limit exemption. Plus the bridge administrator (`bridge.permissions`, prefilled with the operator signing in; everyone else on this server is a `user`).
...
7. **Created.** A runbook (`/bridges/:id/created`), numbered: **Save the files** (for self-managed: `config.yaml` where the render writes one, `registration.yaml`, `docker-compose.yaml`, with where to put them; for Kubernetes: the `Bridge` resource and the registration; each block Copy and Download; a banner that the tokens are shown once and can be rotated later), **Start the bridge** (the exact `docker compose up -d <id>` or `kubectl apply` line), **Watch it connect** (a live status card polling every five seconds: waiting for the first ping, then **Connected** with the ping time; down or degraded with the error and where the log is), **Sign in** (the catalogue's steps for this bridge with the bot's real Matrix ID, the documentation's caveats, and the documentation link). "Open bridge" (a link) goes to the detail page. On a later visit the files are gone (shown once) and the other three steps still hold.
8. **Detail.** The bridge detail page with the catalogue's glyph and name, and a **Sign in** tab that gives the steps for this bridge (the same guide as step 7) or, for an appservice not added through the catalogue, says so and points at the usual `login` command.
