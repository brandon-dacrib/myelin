...
**D6b. Malware scanning is a swappable provider, not a built-in.** Operators choose their scanner and change it without changing the server. ICAP is the primary adapter because nearly everything reaches it: ClamAV through c-icap, the commercial engines natively, and cloud APIs through gateways such as ICAPeg. A direct clamd client exists as an optimisation for operators who would rather not also run c-icap, and an HTTP contract with submit-and-poll covers cloud services with no ICAP fronting, such as CrowdStrike Falcon. The ICAP client is therefore where the effort goes, including preview negotiation and using the protocol's own ISTag as the cache's engine version. Scanning applies to local uploads, asynchronous upload completion, remote media fetched over federation, and bridge uploads, with verdicts cached by content hash and engine version. Modes are block, defer, quarantine or off, and enabling scanning without choosing fail-open or fail-closed is a configuration error, because that is a policy decision. Encrypted media is reported unscannable rather than clean, and the documentation says so, since an operator who believes a scanner covers encrypted rooms has bought a false assurance. Design in `docs/rfcs/0008-content-scanning.md`.
...
| `hs-identity` | Optional Identity Service API component. |
| `hs-pushgw` | Optional Push Gateway component (APNs, FCM, WebPush, UnifiedPush). |
...
3. **All five APIs.** Client-Server and Server-Server and Application Service are the homeserver. Identity Service (`hs identity`) and Push Gateway (`hs push-gateway`, with APNs, FCM, WebPush and UnifiedPush) are optional components of the same binary so a complete self-hosted stack does not need Sydent or Sygnal.
